import {fakeToken, tokenStorage, url} from "../sportsModule/stateReuse";
import {getToken} from "../api/authApi";
import {getJogs, setJog} from "../api/dataApi";

describe('test api request', () => {
    beforeAll(()=>{
        localStorage.setItem(tokenStorage, fakeToken)
    })
    it(`${url}/auth/user should return new token`, async () => {
        const api = await getToken()
        expect(api.length).toBe(64)
        expect(typeof api).toBe('string')
    })
    it(`${url}/data/sync should return new jogs with unique id and same user_id.
    ...
    it(`${url}/data/jog should return new jog item={date,distance,id,time,user_id}`, async () => {
